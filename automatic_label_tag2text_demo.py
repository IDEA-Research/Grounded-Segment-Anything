...
def git_clone(repo='positive666/Tag2Text', branch='main'):
    import os
    if not os.path.exists('Tag2Text'):  
        from subprocess import check_output
        url = f'https://github.com/{repo}'
        check_output(f'git clone {url}.git -b {branch}', shell=True)  
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tag2text_checkpoint", type=str,default='Tag2Text/pretrained/tag2text_swin_14m.pth', help="path to checkpoint file"
    )
    ...
    print('clone and pull git repo https://huggingface.co/spaces/xinyu1205/Tag2Text') 
    git_clone(repo='xinyu1205/Tag2Text')
    print('<info> if transformers version not compatible, please pip install transformers==4.15.0  ')
    ...
    tag2text_model = tag2text.tag2text_caption(pretrained=tag2text_checkpoint,
                                        image_size=384,
                                        vit='swin_b',
                                        delete_tag_index=delete_tag_index).to(device).eval()
