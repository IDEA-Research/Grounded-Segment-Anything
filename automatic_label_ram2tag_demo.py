...
import sys
sys.path.append('Recognize_Anything_Tag2Text')
from Recognize_Anything_Tag2Text.models.tag2text import ram
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ramtag_checkpoint", type=str, required=True, help="path to checkpoint file"
    )
    parser.add_argument(
    ...
    ramtag_checkpoint = args.ramtag_checkpoint  # change the path of the model
    ...
    model_ram = ram(pretrained=ramtag_checkpoint,
                             image_size=384,
                             vit='swin_l')
    model_ram.eval()
    ...
    res = ram_inference(raw_image , model_ram)
    ...
    plt.title('RAM-Tagging ' + text_prompt + '\n')
    plt.axis('off')
    plt.savefig(
        os.path.join(output_dir, "automatic_label_%s.jpg"%(image_path.split('/')[-1].split('.')[0])), 
